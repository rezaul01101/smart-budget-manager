...
function Profile() {
    ...
    const [editField, setEditField] = useState<EditField>({ name: false, email: false });
    const [editedValues, setEditedValues] = useState({ name: userInfo?.name || "", email: userInfo?.email || "" });
    const [showPasswordModal, setShowPasswordModal] = useState(false);
    const [passwordData, setPasswordData] = useState({ currentPassword: "", newPassword: "", confirmPassword: "" });
    const [passwordError, setPasswordError] = useState("");
    const [avatarInitial, setAvatarInitial] = useState(userInfo?.name?.charAt(0).toUpperCase() || "U");
            ...
            <div className="mt-6 flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-shrink-0">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-lg">
                        <span className="text-3xl md:text-4xl font-bold text-white">{avatarInitial}</span>
                    </div>
                </div>
